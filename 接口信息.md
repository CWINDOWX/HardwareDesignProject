...
> 说明：以下模块目前作为“可独立单测的功能块”存在，尚未接入 `mips/controller/datapath` 主流水线；接入工作需要与其他成员协同完成。
...
module DM (
    ...
    input  [2:0]  mem_op_i,
    output [31:0] rdata_o,
    output        adel_o,  // AdEL：读数据地址错
    output        ades_o   // AdES：写数据地址错
);
```

- `adel_o/ades_o`：按 A03 5.1.5，对 `LW/SW/LH/LHU/SH` 进行对齐检查（字访问 `addr[1:0]==0`，半字访问 `addr[0]==0`）。
- `ades_o==1` 时禁止本拍写入（模块内部已屏蔽写使能）。

### 10.CP0_Reg.v（CP0：BadVAddr/Count/Status/Cause/EPC）
...
module CP0_Reg (
    ...
    // exception/interrupt from pipeline
    input         exc_valid_i,
    input  [4:0]  exc_code_i,
    input  [31:0] pc_i,
    input         in_delay_slot_i,
...
- `Status.Bev` 恒为 1；`IM[15:8]/EXL/IE` 可由 `MTC0` 写入（其余位读为 0）。
- `Count` 按 A03 6.3：**每两个 CPU Clock 自增 1**，且支持 `MTC0` 写入。
- `Cause`：仅 `IP1..IP0` 允许软件写；`BD/ExcCode` 只在 `EXL=0` 的例外入口更新。
- `EPC`：例外入口 `EXL=0` 时更新；若 `in_delay_slot_i=1`，记录 `pc_i-4`；`ERET` 清 `EXL`。
